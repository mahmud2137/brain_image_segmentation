...
def image_to_patches(img, patch_shape):
    """
    Crop an image into elementery patches
    Parameters:
    ------------
    img: (xlen, ylen, zlen, n_channels) np.ndarray
        input image to extract patch from
    patch_shape: (xlen, ylen, zlen) np.ndarray
        Patch length in every dimensions
    Image shape must be whole multiple of the patch shape in every dimension
    """
    img_shape = img.shape
    img_patches = []
    img_slice_index_centers = np.array([np.arange(patch_len/2, img_len, patch_len) 
                                    for img_len, patch_len in zip(img_shape, patch_shape)])
    img_slice_index_centers = np.int32(img_slice_index_centers)
    for i in img_slice_index_centers[0]:
        for j in img_slice_index_centers[1]:
            for k in img_slice_index_centers[2]:
                
                img_slices = [
                    slice(c-patch_len//2, c+patch_len//2) for c, patch_len in zip([i,j,k], patch_shape)
                ]
                
                img_patch = img[img_slices]
